...
function currentLevel() {
  return LEVELS[readConfig().logLevel] ?? LEVELS.info;
}
...
const session = { calls: [], inputTokens: 0, outputTokens: 0, cost: 0 };
...
const log = {
  divider: (label = "") => {
    if (!at("info")) return;
    if (label) {
      console.log(`${c.gray}── ${c.cyan}${c.bold}${label}${c.reset} ${c.gray}${"─".repeat(38 - label.length)}${c.reset}`);
    } else {
